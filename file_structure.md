...
该文件夹是一个用于 **SillyTavern** 的第三方扩展插件，名为 **“阅读”**（TwT）。其核心功能是将 SillyTavern 原本的上下滚动阅读模式转换为左右翻页阅读模式，并在此基础上提供了丰富的美化排版、章节目录、分段编辑和段评等功能。
...
*   **[manifest.json](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/manifest.json)**
    *   **掌管功能**：插件的元数据配置文件。
    *   **详细职责**：定义插件在 SillyTavern 中的显示名称（“阅读”）、作者、版本号、最低客户端版本限制，并指定主 JavaScript 入口文件（`index.js`）和主样式表入口文件（`style.css`）。
...
*   **[index.html](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/index.html)**
    ...
    *   **详细职责**：定义 SillyTavern 侧边栏/扩展设置区域的 UI，采用多 Tab 设计（设置、翻页、菜单、视觉、目录、段评），以及段评全屏编辑器的弹窗界面。
*   **[style.css](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/style.css)**
    *   **掌管功能**：插件样式总入口。
    *   **详细职责**：不包含具体样式代码，通过 `@import` 将各个功能模块的子样式表（`pagination.css`、`visual.css`、`menu.css`、`paragraph.css`）进行聚合引入。
...
### 2. 翻页逻辑模块 (Pagination)
*   **[pagination.js](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/pagination.js)**
...
*   **[pagination.css](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/pagination.css)**
    *   **掌管功能**：翻页模式的排版样式。
...
### 3. 操作菜单模块 (Menu)
*   **[menu.js](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/menu.js)**
    *   **掌管功能**：消息长按菜单与批量消息管理。
    *   **详细职责**：
...
*   **[menu.css](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/menu.css)** *(注：存在于目录中)*
...
*   **[mulu.js](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/mulu.js)**
    *   **掌管功能**：章节目录（TOC）管理。
    *   **详细职责**：
        *   提供快捷按钮用于快速跳转到聊天的绝对开头、绝对结尾，或呼出目录面板。
...
*   **[paragraph.js](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/paragraph.js)**
    *   **掌管功能**：分段编辑与 AI 段评。
    *   **详细职责**：
...
*   **[paragraph.css](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/paragraph.css)**
    *   **掌管功能**：分段编辑状态与段评侧边栏样式。
    *   **详细职责**：定义段落编辑时的“选中”、“删除线”、“已编辑”的高亮样式；定义悬浮工具栏和编辑弹窗的位置；控制段评弹幕抽屉的显示位置（左/右）、宽度和滑动动画。

### 6. 视觉美化模块 (Visual)
*   **[visual.js](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/visual.js)**
    *   **掌管功能**：视觉排版设置应用。
    *   **详细职责**：在启用视觉美化时，动态将用户在面板中设置的留白（padding）、字号（font-size）、行高（line-height）、首行缩进（text-indent）、字间距等，作为 CSS 变量注入并应用到 `#chat` 容器上。
*   **[visual.css](file:///d:/SillyTavern/public/scripts/extensions/third-party/TwT/visual.css)**
    *   **掌管功能**：视觉排版样式模板。
